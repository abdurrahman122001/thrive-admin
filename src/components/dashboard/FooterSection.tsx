...
const FooterSection: React.FC<FooterSectionProps> = ({
  ...
  const handleEdit = (footer?: FooterData) => {
    ...
    if (footer?.logo) {
      setLogoPreview(`${API_URL}${footer.logo}`);
    } else {
      setLogoPreview(null);
    }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      Object.entries(editingFooter).forEach(([key, value]) => {
        if (
          key !== "id" &&
          key !== "logo" &&
          value !== null &&
      ...
      if (editingFooter.id) {
        ...
        response = await axios.post(
          `${API_URL}/api/footers/${editingFooter.id}`,
          formData,
          {
            headers: {
        ...
        // For create
        response = await axios.post(`${API_URL}/api/footers`, formData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        });
      ...
      if (editingFooter.id) {
        setFooters(
          footers.map((f) => (f.id === editingFooter.id ? response.data : f))
        );
      } else {
      ...
      setError(err.response?.data?.message || "Failed to save footer");
      console.error(err);
    } finally {
      setLoading(false);
    }
          ...
          >
            ...
            <div className="space-y-1">
              <div className="text-xs text-gray-500 flex items-center space-x-2">
                ...
                <span>
                  Copyright: {footer.copyright_text || "Not specified"}
                </span>
              </div>
              <div className="text-xs text-gray-500 flex items-center space-x-2">
                <div className="w-1 h-1 bg-gray-400 rounded-full"></div>
                <span>Designer: {footer.designer_text || "Not specified"}</span>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center p-6 border-b">
              <h2 className="text-2xl font-bold">
                {editingFooter.id ? "Edit Footer" : "Add Footer"}
              </h2>
              <button
                onClick={() => {
                  setShowModal?.(null);
            ...
            <form onSubmit={handleSave} className="p-6 space-y-6">
              ...
              <div>
                ...
                {editingFooter.logo && !logoPreview && (
                  <div className="mt-3">
                    <p className="text-sm text-gray-500">Current logo:</p>
                    <img
                      src={`${API_URL}${editingFooter.logo}`}
                      alt="Current Logo"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditingFooter({
                        ...editingFooter,
                        disclaimer_link: e.target.value,
                      })
                    }
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  />
